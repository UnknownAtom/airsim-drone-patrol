...
- 界面显示任务状态、飞行高度、速度、推理 FPS、系统延迟和当前检测目标；完整采集/解析/推理耗时与丢帧统计保留在终端摘要；
...
前端不再依赖外部参考界面。当前 GUI 采用浅色大圆角扁平风格：纯色背景、白色卡片、浅灰指标块、蓝色主操作、红色停止操作；不使用渐变。左侧为实时前视画面，右侧为任务状态、核心指标、巡航进度、检测目标和操作按钮。

右侧面板自上而下包含：状态卡（状态和相机提示）、飞行指标 2×2 网格（高度/速度/推理帧率/系统延迟）、巡航进度卡（进度条 + 圈数/航点文本 + 航点路线图）、检测目标卡（固定高度滚动列表）和操作卡（多航点巡航/实时视觉检测/停止任务）。当前不再使用独立的“紧凑模式”或底部状态栏；右侧面板始终是主控制区域。窗口初始尺寸默认 `1600×900`，由 `--display-width` 和 `--display-height` 控制。
