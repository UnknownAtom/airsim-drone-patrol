...
一个基于 **AirSim + YOLO** 的无人机仿真验证程序：自动起飞、按航点自主巡航、实时检测地面目标（行人、车辆等），并通过 PyQt6 GUI 实时展示画面、状态和检测结果。
...
- **实时诊断**：终端摘要记录采集/检测/GUI FPS、取图/解析/推理耗时、检测延迟和丢帧；GUI 显示关键运行指标
- **浅色任务控制台**：左侧实时视频区，右侧状态卡、性能指标、巡航进度、检测目标和停止操作
- **自动任务**：AirSim 就绪后自动开始多航点巡航，支持安全停止并降落
...
默认窗口尺寸为 `1600×900`，可以通过参数调整：

```powershell
python simu.py --display-width 1280 --display-height 720 --display-fps 18
```

界面包含：

- 左侧实时前视相机画面、HUD、检测框、类别和置信度；
- 右侧任务状态、采集/推理/GUI FPS、取图与 YOLO 耗时、相机/检测丢帧；
- 巡航进度条与航点路线图；
- 当前画面的检测目标列表；
- 仅保留 `停止任务` 操作按钮，检测框默认显示。

当前 GUI 不使用独立的“紧凑模式”或自动替换状态栏；右侧控制面板始终是主操作区域。窗口高度过小时，Qt 会根据面板内容限制可用的最小尺寸。
